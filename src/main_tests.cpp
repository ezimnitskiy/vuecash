...
static void fix_merge_mining_tag(BlockTemplate &block) {
	if (block.major_version >= 2) {
		vuecash::TransactionExtraMergeMiningTag mmTag;
		mmTag.depth = 0;
		block.parent_block.base_transaction.extra.clear();
		mmTag.merkle_root = get_auxiliary_block_header_hash(block);
		if (!vuecash::append_merge_mining_tag_to_extra(block.parent_block.base_transaction.extra, mmTag))
			throw std::runtime_error("vuecash::append_merge_mining_tag_to_extra failed");
	}
}
...
void test_blockchain(common::CommandLine &cmd) {
	...
	Config config(cmd);
	config.coin_directory = "/home/user/devbox/vuecash/tests";
	vuecash::BlockChain::DB::delete_db(config.coin_directory + "/blockchain");
	...
	for (int i = 0; i != 100; ++i) {
		...
		Height height         = 0;
		if (!block_chain.create_mining_block_template(block, address, vuecash::BinaryArray{}, difficulty, height))
