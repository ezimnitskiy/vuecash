...
/*struct WalletTransactionDto {
        ...
        WalletTransactionDto(const vuecash::WalletTransaction &wallet) {
                state         = wallet.state;
        ...
        vuecash::WalletTransactionState state;
        uint64_t timestamp;
        uint32_t block_height;
        Hash hash;
...
version(version) {}
        WalletTransferDto(const vuecash::WalletTransfer &tr, uint32_t version)
...
/*void serialize(WalletRecordDto &v, vuecash::ISerializer &s) {
...
struct KeysStorage {
	...
	/*	void serialize(vuecash::ISerializer & s, const std::string &name) {
	            s.begin_object(name);
...
/*void serialize(ObsoleteSpentOutputDto &v, vuecash::ISerializer &s) {
        s(v.amount, "amount");
...
void serialize(ObsoleteChangeDto &v, vuecash::ISerializer &s) {
        s(v.tx_hash, "transaction_hash");
        s(v.amount, "amount");
}

void serialize(UnlockTransactionJobDto &v, vuecash::ISerializer &s) {
        s(v.block_height, "block_height");
        s(v.transaction_hash, "transaction_hash");
        s(v.wallet_index, "wallet_index");
}

void serialize(WalletTransactionDto &v, vuecash::ISerializer &s) {
        typedef std::underlying_type<vuecash::WalletTransactionState>::type
StateType;

        StateType state = static_cast<StateType>(v.state);
        s(state, "state");
        v.state = static_cast<vuecash::WalletTransactionState>(state);

        s(v.timestamp, "timestamp");
        vuecash::serializeBlockHeight(s, v.block_height, "block_height");
        s(v.hash, "hash");
...
void serialize(WalletTransferDto &v, vuecash::ISerializer &s) {
        s(v.address, "address");
        s(v.amount, "amount");
...
std::string readCipher(common::IInputStream &source, const std::string &name) {
	std::string cipher;
	//	vuecash::BinaryInputStreamSerializer s(source);
...
std::string decrypt(const std::string &cipher, vuecash::WalletSerializerV1::CryptoContext &crypto_ctx) {
	std::string plain;
...
void deserialize(Object &obj, const std::string &name, const std::string &plain) {
	...
	//	vuecash::BinaryInputStreamSerializer s(stream);
	s(obj);  // , common::StringView(name)
}

template<typename Object>
void deserializeEncrypted(Object &obj, const std::string &name, vuecash::WalletSerializerV1::CryptoContext &crypto_ctx,
...
/*vuecash::WalletTransaction convert(const vuecash::WalletLegacyTransaction
&tx) {
        vuecash::WalletTransaction mtx;

        mtx.state         = vuecash::WalletTransactionState::SUCCEEDED;
...
/*vuecash::WalletTransfer convert(const vuecash::WalletLegacyTransfer &tr) {
        vuecash::WalletTransfer mtr;
...
void WalletSerializerV1::load_wallet_v1(common::IInputStream &source, const crypto::chacha8_key &key) {
	...
	//	vuecash::BinaryInputStreamSerializer encrypted(source);
	...
	//	vuecash::BinaryInputStreamSerializer serializer(decrypted_stream);
...
/*void
WalletSerializerV1::loadWalletV1Details(vuecash::BinaryInputStreamSerializer&
serializer) {
...
void WalletSerializerV1::load_wallets(common::IInputStream &source, CryptoContext &crypto_ctx) {
	...
	for (uint64_t i = 0; i < count; ++i) {
		...
		//		wallet.container =
		// nullptr;//reinterpret_cast<vuecash::ITransfersContainer*>(i); //dirty
		// hack.
		// container
