...
static void parse_peer_and_add_to_container(std::string str, std::vector<vuecash::NetworkAddress> &container) {
	vuecash::NetworkAddress na{};
...
const static UUID VUECASH_NETWORK = {{0x1A, 0x10, 0xF1, 0x11, 0x11, 0xF0, 0x01, 0x01, 0x1A, 0x11, 0x12, 0xA1, 0x10,
    0x12, 0xA1, 0x10}};  // Bender's nightmare
...
Config::Config(common::CommandLine &cmd)
    ...
    , network_id(VUECASH_NETWORK)
    , p2p_bind_port(P2P_DEFAULT_PORT)
    , p2p_external_port(P2P_DEFAULT_PORT)
    , p2p_bind_ip("0.0.0.0")
    , vuecashd_bind_port(RPC_DEFAULT_PORT)
    , vuecashd_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , vuecashd_remote_port(0)
    , vuecashd_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--vuecashd-authorization")) {
		// go to https://www.base64encode.org and encode username:password there
		vuecashd_authorization = pa;
	}
	if (const char *pa = cmd.get("--vuecashd-bind-address")) {
		if (!common::parse_ip_address_and_port(vuecashd_bind_ip, vuecashd_bind_port, pa))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--vuecashd-remote-address")) {
		const std::string prefix = "https://";
		if (std::string(pa).find(prefix) == 0) {
#if !VUECASH_SSL
			throw std::runtime_error("Using https in --vuecashd-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			vuecashd_remote_port = boost::lexical_cast<uint16_t>(sport);
			vuecashd_remote_ip = prefix + sip;
		}else {
			if (!common::parse_ip_address_and_port(vuecashd_remote_ip, vuecashd_remote_port, pa))
				throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
		}
	}
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : vuecash::SEED_NODES) {
			NetworkAddress addr;
