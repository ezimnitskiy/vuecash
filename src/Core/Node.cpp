...
Node::Node(logging::ILogger &log, const Config &config, BlockChainState &block_chain)
	...
	if (!config.vuecashd_bind_ip.empty() && config.vuecashd_bind_port != 0)
		m_api.reset(new http::Server(config.vuecashd_bind_ip, config.vuecashd_bind_port,
	...
	m_commit_timer.once(DB_COMMIT_PERIOD_VUECASHD);
	advance_long_poll();
}
...
void Node::sync_transactions(P2PClientVuecash *who) {
...
void Node::P2PClientVuecash::on_msg_bytes(size_t, size_t) {  // downloaded. uploaded
	//    node->peers.on_peer_bytes(get_address(), downloaded, uploaded,
	//    node->p2p.get_local_time());
}

CORE_SYNC_DATA
Node::P2PClientVuecash::get_sync_data() const {
...
std::vector<PeerlistEntry> Node::P2PClientVuecash::get_peers_to_share() const {
	auto result =
	    m_node->m_peer_db.get_peerlist_to_p2p(m_node->m_p2p.get_local_time(), config.p2p_default_peers_in_handshake);
	return result;
}

void Node::P2PClientVuecash::on_first_message_after_handshake() {
...
void Node::P2PClientVuecash::on_msg_handshake(COMMAND_HANDSHAKE::request &&req) {
	NetworkAddress addr;
...
void Node::P2PClientVuecash::on_msg_handshake(COMMAND_HANDSHAKE::response &&req) {
...
void Node::P2PClientVuecash::on_msg_notify_request_chain(NOTIFY_REQUEST_CHAIN::request &&req) {
...
void Node::P2PClientVuecash::on_msg_notify_request_chain(NOTIFY_RESPONSE_CHAIN_ENTRY::request &&req) {
	m_node->m_downloader.on_msg_notify_request_chain(this, req);
}

void Node::P2PClientVuecash::on_msg_notify_request_objects(NOTIFY_REQUEST_GET_OBJECTS::request &&req) {
...
void Node::P2PClientVuecash::on_msg_notify_request_objects(NOTIFY_RESPONSE_GET_OBJECTS::request &&req) {
	m_node->m_downloader.on_msg_notify_request_objects(this, req);
}

void Node::P2PClientVuecash::on_disconnect(const std::string &ban_reason) {
...
void Node::P2PClientVuecash::on_msg_notify_request_tx_pool(NOTIFY_REQUEST_TX_POOL::request &&req) {
...
void Node::P2PClientVuecash::on_msg_timed_sync(COMMAND_TIMED_SYNC::request &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}
void Node::P2PClientVuecash::on_msg_timed_sync(COMMAND_TIMED_SYNC::response &&req) {
	m_node->m_downloader.on_msg_timed_sync(req.payload_data);
}

void Node::P2PClientVuecash::on_msg_notify_new_block(NOTIFY_NEW_BLOCK::request &&req) {
...
void Node::P2PClientVuecash::on_msg_notify_new_transactions(NOTIFY_NEW_TRANSACTIONS::request &&req) {
...
void Node::P2PClientVuecash::on_msg_network_state(COMMAND_REQUEST_NETWORK_STATE::request &&req) {
...
void Node::P2PClientVuecash::on_msg_stat_info(COMMAND_REQUEST_STAT_INFO::request &&req) {
	if (!m_node->check_trust(req.tr)) {
		disconnect(std::string());
		return;
	}
...
void Node::advance_long_poll() {
	...
	if (m_long_poll_http_clients.empty())
		return;
	api::vuecashd::GetStatus::Response resp = create_status_response3();
	...
	for (auto lit = m_long_poll_http_clients.begin(); lit != m_long_poll_http_clients.end();) {
		const bool method_status = lit->original_json_request.get_method() == api::vuecashd::GetStatus::method() ||
		                           lit->original_json_request.get_method() == api::vuecashd::GetStatus::method2();
		...
		if (method_status) {
			...
			try {
				api::vuecashd::GetBlockTemplate::Request gbt_req;
				lit->original_json_request.load_params(gbt_req);
				api::vuecashd::GetBlockTemplate::Response gbt_res;
...
</svg></td><td>vuecashd &bull; version
)";
...
bool Node::on_api_http_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	if (!m_config.vuecashd_authorization.empty() &&
	    request.r.basic_authorization != m_config.vuecashd_authorization) {
...
std::unordered_map<std::string, Node::HTTPHandlerFunction> Node::m_http_handlers = {
    ...
    {api::vuecashd::SyncBlocks::binMethod(), binMethod2(&Node::on_wallet_sync3)},
    {api::vuecashd::SyncMemPool::binMethod(), binMethod2(&Node::on_sync_mempool3)},
...
std::unordered_map<std::string, Node::JSONRPCHandlerFunction> Node::m_jsonrpc_handlers = {
    {api::vuecashd::GetBlockTemplate::method(), json_rpc::makeMemberMethodSeria(&Node::on_getblocktemplate)},
    ...
    {api::vuecashd::GetStatus::method(), json_rpc::makeMemberMethodSeria(&Node::on_get_status3)},
    {api::vuecashd::GetStatus::method2(), json_rpc::makeMemberMethodSeria(&Node::on_get_status3)},
    {api::vuecashd::SendTransaction::method(), json_rpc::makeMemberMethodSeria(&Node::handle_send_transaction3)},
    {api::vuecashd::CheckSendProof::method(), json_rpc::makeMemberMethodSeria(&Node::handle_check_send_proof3)},
    {api::vuecashd::SyncBlocks::method(), json_rpc::makeMemberMethodSeria(&Node::on_wallet_sync3)},
    {api::vuecashd::SyncMemPool::method(), json_rpc::makeMemberMethodSeria(&Node::on_sync_mempool3)}};

bool Node::on_get_random_outputs3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::vuecashd::GetRandomOutputs::Request &&request, api::vuecashd::GetRandomOutputs::Response &response) {
...
api::vuecashd::GetStatus::Response Node::create_status_response3() const {
	api::vuecashd::GetStatus::Response res;
...
bool Node::on_get_status3(http::Client *who, http::RequestData &&raw_request, json_rpc::Request &&raw_js_request,
    api::vuecashd::GetStatus::Request &&req, api::vuecashd::GetStatus::Response &res) {
...
bool Node::on_wallet_sync3(http::Client *, http::RequestData &&, json_rpc::Request &&json_req,
    api::vuecashd::SyncBlocks::Request &&req, api::vuecashd::SyncBlocks::Response &res) {
	...
	if (req.max_count > api::vuecashd::SyncBlocks::Request::MAX_COUNT) {
...
bool Node::on_sync_mempool3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::vuecashd::SyncMemPool::Request &&req, api::vuecashd::SyncMemPool::Response &res) {
...
bool Node::handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::vuecashd::SendTransaction::Request &&request, api::vuecashd::SendTransaction::Response &response) {
	NOTIFY_NEW_TRANSACTIONS::request msg;
	Transaction tx;
...
bool Node::handle_check_send_proof3(http::Client *, http::RequestData &&, json_rpc::Request &&,
    api::vuecashd::CheckSendProof::Request &&request, api::vuecashd::CheckSendProof::Response &response) {
	Transaction tx;
	SendProof sp;
	try {
