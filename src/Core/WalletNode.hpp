...
public:
	...
	bool handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
	    api::vuecashd::SendTransaction::Request &&,
	    api::vuecashd::SendTransaction::Response &);  // We lock spent outputs until next pool sync
...
private:
	...
	struct LongPollClient {
		...
		http::RequestData original_request;
		json_rpc::OptionalJsonValue original_jsonrpc_id;
		vuecash::api::walletd::GetStatus::Request original_get_status;
	};
