...
Hash vuecash::get_base_transaction_hash(const BaseTransaction &tx) {
	if (tx.version < 2)
...
void vuecash::decompose_amount(Amount amount, Amount dustThreshold, std::vector<Amount> &decomposedAmounts) {
...
size_t vuecash::get_maximum_tx_size(size_t inputCount, size_t outputCount, size_t mixinCount) {
...
bool vuecash::get_tx_fee(const Transaction &tx, uint64_t &fee) {
...
uint64_t vuecash::get_tx_fee(const Transaction &tx) {
	uint64_t r = 0;
	if (!get_tx_fee(tx, r))
		return 0;
	return r;
