...
public:
	...
	bool parse(const BinaryArray &extra) {
		m_fields.clear();
		return vuecash::parse_transaction_extra(extra, m_fields);
	}
	template<typename T>
	...
	bool get_public_key(crypto::PublicKey &pk) const {
		vuecash::TransactionExtraPublicKey extra_pk;
		if (!get(extra_pk)) {
			return false;
		}
...
private:
	std::vector<vuecash::TransactionExtraField>::const_iterator find(const std::type_info &t) const {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const vuecash::TransactionExtraField &f) { return t == f.type(); });
	}
	std::vector<vuecash::TransactionExtraField>::iterator find(const std::type_info &t) {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const vuecash::TransactionExtraField &f) { return t == f.type(); });
	}

	std::vector<vuecash::TransactionExtraField> m_fields;
};
}

namespace seria {
class ISeria;
void ser(vuecash::TransactionExtraMergeMiningTag &v, ISeria &s);
}
