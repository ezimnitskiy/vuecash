...
/*bool createTxExtraWithPaymentId(const std::string &paymentIdString, BinaryArray &extra) {
    ...
    vuecash::set_payment_id_to_transaction_extra_nonce(extraNonce, paymentIdBin);

    if (!vuecash::add_extra_nonce_to_transaction_extra(extra, extraNonce)) {
        return false;
    }
...
static void doSerialize(vuecash::TransactionExtraMergeMiningTag &tag, seria::ISeria &s) {
	s.begin_object();
...
void seria::ser(vuecash::TransactionExtraMergeMiningTag &v, ISeria &s) {
	if (s.is_input()) {
		std::string field;
		s(field);
