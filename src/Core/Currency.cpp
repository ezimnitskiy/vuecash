...
uint32_t Currency::block_granted_full_reward_zone_by_block_version(uint8_t block_major_version) const {
	...
	if (block_major_version == 2)
		return vuecash::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V2;
	return vuecash::parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1;
}
...
Hash vuecash::get_transaction_inputs_hash(const TransactionPrefix &tx) {
	BinaryArray ba = seria::to_binary(tx.inputs);
	Hash new_hash  = crypto::cn_fast_hash(ba.data(), ba.size());
	return new_hash;
}

Hash vuecash::get_transaction_prefix_hash(const TransactionPrefix &tx) {
...
Hash vuecash::get_transaction_hash(const Transaction &tx) {
...
Hash vuecash::get_block_hash(const BlockTemplate &bh) {
...
Hash vuecash::get_auxiliary_block_header_hash(const BlockTemplate &bh) {
	return get_object_hash(get_block_hashing_binary_array(bh));
}

Hash vuecash::get_block_long_hash(const BlockTemplate &bh, crypto::CryptoNightContext &crypto_ctx) {
