...
crypto::Hash vuecash::proof_of_trust::get_hash() const {
	std::string s;
...
// P2pProtocolTypes
void ser(vuecash::UUID &v, seria::ISeria &s) { s.binary(&v, sizeof(v)); }

void ser_members(vuecash::PeerlistEntry &v, seria::ISeria &s) {
...
void ser_members(vuecash::NetworkAddress &v, seria::ISeria &s) {
	seria_kv("ip", v.ip, s);
	seria_kv("port", v.port, s);
}

void ser_members(vuecash::connection_entry &v, seria::ISeria &s) {
	seria_kv("adr", v.adr, s);
	seria_kv("id", v.id, s);
	seria_kv("is_income", v.is_income, s);
}

void ser_members(vuecash::CoreStatistics &v, seria::ISeria &s) {
...
void ser_members(vuecash::network_config &v, seria::ISeria &s) {
...
void ser_members(vuecash::basic_node_data &v, seria::ISeria &s) {
...
void ser_members(vuecash::CORE_SYNC_DATA &v, seria::ISeria &s) {
	if (s.is_input()) {
		uint32_t on_wire = 0;
...
void ser_members(vuecash::COMMAND_HANDSHAKE::request &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(vuecash::COMMAND_HANDSHAKE::response &v, seria::ISeria &s) {
	seria_kv("node_data", v.node_data, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(vuecash::COMMAND_TIMED_SYNC::request &v, seria::ISeria &s) {
	seria_kv("payload_data", v.payload_data, s);
}

void ser_members(vuecash::COMMAND_TIMED_SYNC::response &v, seria::ISeria &s) {
	seria_kv("local_time", v.local_time, s);
	seria_kv("payload_data", v.payload_data, s);
	serialize_as_binary(v.local_peerlist, "local_peerlist", s);
}

void ser_members(vuecash::COMMAND_PING::request &v, seria::ISeria &s) {}

void ser_members(vuecash::COMMAND_PING::response &v, seria::ISeria &s) {
	seria_kv("status", v.status, s);
	seria_kv("peer_id", v.peer_id, s);
}

#ifdef ALLOW_DEBUG_COMMANDS
void ser_members(vuecash::proof_of_trust &v, seria::ISeria &s) {
	seria_kv("peer_id", v.peer_id, s);
	seria_kv("time", v.time, s);
	seria_kv("sign", v.sign, s);
}

void ser_members(vuecash::COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(vuecash::COMMAND_REQUEST_STAT_INFO::response &v, seria::ISeria &s) {
	seria_kv("version", v.version, s);
...
void ser_members(vuecash::COMMAND_REQUEST_NETWORK_STATE::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }

void ser_members(vuecash::COMMAND_REQUEST_NETWORK_STATE::response &v, seria::ISeria &s) {
...
void ser_members(vuecash::RawBlockLegacy &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
		    [](const std::string &s) { return vuecash::BinaryArray(s.data(), s.data() + s.size()); });
	} else {
		...
		std::transform(v.transactions.begin(), v.transactions.end(), std::back_inserter(other_transactions),
		    [](vuecash::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("block", other_block, s);
		seria::seria_kv("txs", other_transactions, s);
	}
}

void ser_members(vuecash::NOTIFY_NEW_BLOCK::request &v, seria::ISeria &s) {
	seria_kv("b", v.b, s);
	seria_kv("current_blockchain_height", v.current_blockchain_height, s);
	seria_kv("hop", v.hop, s);
}

void ser_members(vuecash::NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(v.txs.begin(), v.txs.end(), std::back_inserter(transactions),
		    [](const vuecash::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("txs", transactions, s);
	}
}

void ser_members(vuecash::NOTIFY_REQUEST_GET_OBJECTS::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
	serialize_as_binary(v.blocks, "blocks", s);
}

void ser_members(vuecash::NOTIFY_RESPONSE_GET_OBJECTS::request &v, seria::ISeria &s) {
...
void ser_members(vuecash::NOTIFY_REQUEST_CHAIN::request &v, seria::ISeria &s) {
	serialize_as_binary(v.block_ids, "block_ids", s);
}

void ser_members(vuecash::NOTIFY_RESPONSE_CHAIN_ENTRY::request &v, seria::ISeria &s) {
...
void ser_members(vuecash::NOTIFY_REQUEST_TX_POOL::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
}
}
