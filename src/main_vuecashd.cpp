...
#endif
    R"(  --vuecashd-authorization=<auth>   HTTP Basic Authorization header (base64 of login:password)
)";
...
int main(int argc, const char *argv[]) try {
	...
	platform::ExclusiveLock coin_lock(coinFolder, "vuecashd.lock");

	logging::LoggerManager logManager;
	logManager.configure_default(config.get_coin_directory("logs"), "vuecashd-");
	...
	std::cout << "vuecashd started seconds=" << double(idea_ms.count()) / 1000 << std::endl;
	while (!io.stopped()) {
