...
Options:
  ...
  --vuecashd-remote-address=<ip:port> Connect to remote vuecashd and suppress running built-in vuecashd.
  --vuecashd-authorization=<auth>     HTTP Basic Authorization header (base64 of login:password)

Options for built-in vuecashd (run when no --vuecashd-remote-address specified):
  ...
  --vuecashd-bind-address=<ip:port>   Interface and port for vuecashd RPC [default: 0.0.0.0:8081].
...
static const bool separate_thread_for_vuecashd = true;
...
int main(int argc, const char *argv[]) try {
	...
	vuecash::Config config(cmd);
	vuecash::Currency currency(config.is_testnet);

	if (cmd.should_quit(USAGE, vuecash::app_version()))
		return 0;
	logging::LoggerManager logManagerNode;
	logManagerNode.configure_default(config.get_coin_directory("logs"), "vuecashd-");
	...
	try {
		if (!config.vuecashd_remote_port)
			blockchain_lock = std::make_unique<platform::ExclusiveLock>(coinFolder, "vuecashd.lock");
	} catch (const platform::ExclusiveLock::FailedToLock &ex) {
		std::cout << ex.what() << std::endl;
		return api::VUECASHD_ALREADY_RUNNING;
	}
	try {
	...
	std::thread vuecashd_thread;
	if (!config.vuecashd_remote_port) {
		try {
			if (separate_thread_for_vuecashd) {
				vuecashd_thread = std::thread([&prm, &logManagerNode, &config, &currency] {
					boost::asio::io_service io;
			...
			std::cout << ex.what() << std::endl;
			if (vuecashd_thread.joinable())
				vuecashd_thread.join();  // otherwise terminate will be called in ~thread
			return api::VUECASHD_BIND_PORT_IN_USE;
		}
	}
